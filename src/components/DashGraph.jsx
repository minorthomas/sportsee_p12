...
import { AverageGraph } from './Graphics/AverageGraph';
import { SessionsGraph } from './Graphics/SessionsGraph';
import { PerformanceGraph } from './Graphics/PerformanceGraph';
import { ScoreGraph } from './Graphics/ScoreGraph';
import { USER_MAIN_DATA } from '../data/__mocks__/mockedData';
import { DashCard } from './DashCard/DashCard';

export function DashboardGraph() {
    const data = USER_MAIN_DATA[1].keyData;

    const keyData = [
        {
            value: data.calorieCount ? data.calorieCount : 'No data',
            type: 'Calories',
            color: 'red',
            unit: data.calorieCount ? 'kCal' : ' ',
            icon: (
                <svg
                    width='17'
                    height='20'
        ...
        {
            value: data.proteinCount ? data.proteinCount : 'No data',
            type: 'Proteines',
            color: 'blue',
            unit: data.proteinCount ? 'g' : '',
            icon: (
                <svg
                    width='19'
                    height='19'
        ...
        {
            value: data.carbohydrateCount ? data.carbohydrateCount : 'No data',
            type: 'Glucides',
            color: 'yellow',
            unit: data.carbohydrateCount ? 'g' : '',
            icon: (
                <svg
                    width='18'
                    height='20'
        ...
        {
            value: data.lipidCount ? data.lipidCount : 'No data',
            type: 'Lipides',
            color: 'pink',
            unit: data.lipidCount ? 'g' : '',
            icon: (
                <svg
                    width='20'
                    height='19'
